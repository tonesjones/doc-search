#!/usr/bin/env python3
"""Scrape pending Coverity doc topics via Fluid Topics content API.

Examples:
  python scripts/scrape-pending.py --product coverity-2026.6 --all-pending
  python scripts/scrape-pending.py --section "Coverity overview" --all-pending
  python scripts/scrape-pending.py --section "Coverity Analysis" --exclude-path "Customization guides"
  python scripts/scrape-pending.py --path-contains "Coverity Connect" --limit 20
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape pending Coverity topics")
    parser.add_argument(
        "--product",
        type=str,
