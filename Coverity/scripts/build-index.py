...
Usage:
  python scripts/build-index.py                         # rebuild Coverity index from manifest
...
def write_hub_index() -> None:
    ...
    lines: list[str] = []
    a = lines.append
    a("# Coverity Documentation Corpus")
    a("")
    a(
        "> Local knowledge base for RAG (Coverity Documentation). "
    ...
    a("")
    a("```powershell")
    a("python scripts/build-index.py --product coverity-2026.6 --init")
    a("python scripts/scrape-pending.py --product coverity-2026.6 --all-pending")
    a("python scripts/build-index.py --product coverity-2026.6 --hub")
    a("```")
    a("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Coverity docs index/manifest")
    parser.add_argument(
        "--product",
        type=str,
