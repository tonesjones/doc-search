...
content_id: "Z_Nl6U~~4P9ALJAB5oFYsg"
version: "2026.6"
section: "Coverity Analysis"
scraped_at: "2026-08-12T19:40:30.726432+00:00"
---

# Options

--addmodule <file>
:   Identifies a module that is referenced by the compilation but not added to
    the emit repository. The location of the module can be absolute or relative.
    For rules on specifying the location, see --reference.
...
--allow-missing-refs
:   Issues a warning if any referenced assemblies are missing. If you do not set
    this option, missing assemblies result in an error that stops the process.
    This error applies to explicitly referenced assemblies (see --reference and --addmodule) that are absolute or not found in the Common
    Language Runtime (CLR) system directory. The error also applies to
    mscorlib.dll (see --nostdlib). Note that missing csc.rsp
    assemblies (see --noconfig) always result in a warning.

--codepage <codepage>
:   Identifies the numeric codepage corresponding to codepages that are supported
    by CSC with the `/codepage` option. Source file encodings are
...
:   Identifies the CLR system directory. It is an error to specify a directory that does not exist.
    The CLR system directory is used as a search path for referenced assemblies
    (see `--reference`) and to locate the
    csc.rsp file (see `--noconfig`). If
    `--compiler-dir` is not specified, the command defaults
    to
    $SYSTEM_ROOT/Microsoft.NET/Framework/<version>,
    where `<version>` is the latest supported framework
    version (for details, see the Coverity 2026.6.0 Installation and Upgrade Guide). It is
    an error if no suitable CLR system directory is found.

--define <define>
:   Corresponds to the CSC preprocessor directive and `/define`
...
:   By default, the `cov-emit-cs` command attempts to replace
...
:   Makes `cov-emit-cs` fall back to error recovery mode when
...
:   Specifies the C# Language version to use. Corresponds to the CSC option
    `/langversion`.
...
:   Identifies a library directory to use when searching for referenced
    ...
    corresponds to the CSC `/lib` option.
...
:   Supercedes `--use-link-semantics`. Effectively the same as
    ...
    certain COM interop types. Corresponds to the CSC option
    `/link`.

--noconfig
:   Ignores the csc.rsp file under the CLR system directory
    (see `--compiler-dir`). If this option is not set, the
    references `/r` or `/reference` within
    csc.rsp are added to the list of referenced
    assemblies. Any csc.rsp references that are not
    absolute filenames are subject to the search directory rules (for details,
    see `--reference`). Corresponds to the CSC option
    `/noconfig`.
...
:   Disables the default behavior of searching for
    ...
    csc.exe.config file that specifies a
...
:   Prevents the compilation from accessing internal types or members. This
    ...
    through the `--out` option. This behavior corresponds to CSC
    behavior in the case where `/out` is not specified and a
    default name is used.

--no-banner
:   Suppresses the `cov-emit-cs` application name and version
    banner.
...
:   Specifies the compiler output file, which is then used by the command to
    ...
    Subsequent calls to `cov-emit-cs` will not re-emit the
...
:   Adds a search path to assist `cov-emit-cs` in finding
    ...
    `cov-emit-cs` at the correct .NET Core framework
    directory in cases where `cov-emit-cs` fails to find the
...
:   Identifies an assembly to provide for compilation and addition to the emit
    ...
    Note that `[alias=]` identifies an extern alias directive,
    just as it does for CSC.
...
--target
:   Specifies the type of assembly that was created. Corresponds to the CSC
    option `/target`.

--unsafe
:   Allows analysis C# constructs that are declared with the
    `unsafe` modifier. This option corresponds to the
    `/unsafe` option to CSC.

--use-link-semantics
:   Corresponds to the `/link` option to CSC, which changes how
    the compiler treats certain COM `interop` types.
