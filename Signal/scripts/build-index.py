...
Usage:
  python scripts/build-index.py                         # rebuild Signal index from manifest
...
def write_hub_index() -> None:
    ...
    lines: list[str] = []
    a = lines.append
    a("# Black Duck Signal Documentation Corpus")
    a("")
    a(
        "> Local knowledge base for RAG (Black Duck Signal). "
    ...
    a("")
    a("```powershell")
    a("python scripts/build-index.py --product signal-latest --init")
    a("python scripts/scrape-pending.py --product signal-latest --all-pending")
    a("python scripts/build-index.py --product signal-latest --hub")
    a("```")
    a("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Signal docs index/manifest")
    parser.add_argument(
        "--product",
        type=str,
