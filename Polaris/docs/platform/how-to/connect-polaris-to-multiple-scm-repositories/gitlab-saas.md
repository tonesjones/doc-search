...
2. Connect to your SCM:
   ...
   2. Select GitLab.
   3. Enter the token created in GitLab, (see Prerequisites) under Repository Access Token.

      Note: The access token provided here will be used to complete the onboarding process and will be subject to rate limits enforced by GitLab.
   4. Click Connect.
...
4. Repository Mapping: This will be different according to which method is selected.
   ...
   1. Automatic Mapping: Lists the group/subgroup/repository names as applications and projects. You can view repositories to be imported by using the pulldowns (read only).
...
3. Connect to your SCM:
   1. Select the type of server that is hosting your repository: Cloud-hosted.
   2. Select GitLab
   3. Enter the access token created in GitLab (see Prerequisites) under Repository Access Token.
   ...
   4. Click Connect.
      ...
      - Check that you selected the correct provider for your source repository.
      - Check that you have authorized use of access token outside SSO (if applicable).
4. Repository Mapping: Select subgroups or repositories to import as new projects from pulldowns. If an arrow is next to a name, you can click to select repositories within it.
