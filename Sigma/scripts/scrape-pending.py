#!/usr/bin/env python3
"""Scrape pending Sigma doc topics via Fluid Topics content API.

Examples:
  python scripts/scrape-pending.py --product sigma-2026.8.0 --all-pending
  python scripts/scrape-pending.py --section "Sigma User Guide" --all-pending
  python scripts/scrape-pending.py --path-contains "Command Reference"
  python scripts/scrape-pending.py --path-contains "Jenkins" --limit 20
...
def empty_leaf_markdown(topic: dict) -> str:
    ...
    return (
        f"# {title}\n\n"
        "This official topic has no extractable body in Black Duck Sigma."
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape pending Sigma topics")
    parser.add_argument(
        "--product",
        type=str,
