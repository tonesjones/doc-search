...
source_url: "https://docs.blackduck.com/r/detect/12.0.0/black-duck-detect/quoting-and-escaping-shell-script-arguments.html"
...
## Running the Bash script (detect12.sh) on Linux or Mac

The recommended environment ("parent shell") for running detect12.sh on Linux is Bash, and Bash or Zsh on Mac.
...
## Running in Command Prompt (cmd) on Windows (detect12.ps1)

On Windows, you can run detect12.ps1 in either a [Windows Command Prompt](https://en.wikipedia.org/wiki/Cmd.exe)
session, or a PowerShell session.
This section describes running
detect12.ps1 in a [Windows Command Prompt](https://en.wikipedia.org/wiki/Cmd.exe)
session.
...
## Running in PowerShell on Windows (detect12.ps1)

An alternative environment for running detect12.ps1 on Windows is to run it from inside a PowerShell session.
