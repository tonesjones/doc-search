---
title: "Black Duck Alert Quickstart (Docker Swarm - External Database)"
source_url: "https://docs.blackduck.com/r/alert/8.4.0/black-duck-alert-user-guide/black-duck-alert-quickstart-docker-swarm-external-database-.html"
content_id: "icDRDi7_3vurAWjvq3APCQ"
version: "8.4.0"
section: "Installing and Upgrading Alert"
scraped_at: "2026-08-08T23:46:24.216972+00:00"
---

# Black Duck Alert Quickstart (Docker Swarm - External Database)

Note: For this quickstart guide, we will be focusing on the integrated deployment method with an externally deployed database.

## Prerequsites

- The Alert deployment [files](https://github.com/blackducksoftware/blackduck-alert/releases)
- An existing Black Duck SCA server with enough free resources to start the containers (If your Black Duck SCA server is already running, stop it before
...
from there. This keeps Alert and Black Duck deployments seperate which is useful
in scenarios where you want to temporarily disable Alert. Alternatively you can
merge the main Black Duck and Alert overrides files.
...
At this point, we need to configure the Black Duck webserver to open up the routing
for UI access. Run the following command within your main Black Duck installation
directory:
...
Note: Failure to apply permissions may cause issues if the Alert container is restarted.
