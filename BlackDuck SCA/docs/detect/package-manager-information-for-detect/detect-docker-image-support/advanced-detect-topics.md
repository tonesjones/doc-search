---
title: "Advanced Detect topics"
source_url: "https://docs.blackduck.com/r/detect/11.5.1/black-duck-detect/advanced-detect-topics.html"
content_id: "9ihq_kHmdnFkXoYQfJCjig"
version: "11.5.1"
section: "Package Manager information for Detect"
scraped_at: "2026-08-08T23:45:00.520563+00:00"
---
...
Detect Docker Inspector discovers dependencies in the target image by making a request to
...
3. Runs its own linux package manager (invoking its "list" function: "dpkg -l", "rpm -qa --qf ...", or "apk info -v") on the package manager
   ...
   again at step 1).
4. Translates that package list to BDIO and uploads it to Black Duck SCA, or returns it to the the
   caller (e.g. Detect) to upload to Black Duck SCA.
...
group ID of the Detect Docker Inspector process. Consequently, the the environment must be confugred
so that files created by Detect Docker Inspector are readable by all. On Linux, this means an appropriate
...
Docker > Settings > Resource > File Sharing. To enable Detect Docker Inspector to mount the volumes
...
Alternatively, you can direct Detect Docker Inspector to organize components by image layer
by setting property `bdio.organize.components.by.layer=true`.
Run this way, Detect Docker Inspector will produce BDIO containing image layers
at the top level of the graph, and components associated with each layer appearing as children of that layer.
This structure is visible in from the Black Duck SCA project version Source display.

A side effect of this components-under-layers graph structure is
the categorization by Black Duck SCA of all components as Transitive.
...
only Black Duck SCA versions 2021.8.0 and newer have the ability to correctly display graphs organized by layer, if *Admin > System Settings > Scan > Component Dependency Duplication Sensitivity* is set high enough to avoid removal of components
...
If you include removed components, Detect Docker Inspector will produce BDIO containing a graph with image layers
...
If you exclude removed components (the default), Detect Docker Inspector will behave as described in the section above
...
the components generated by Detect Docker Inspector contain only components from your application layers.
...
Set the value of the Detect property *detect.docker.platform.top.layer.id* to the platform top layer ID.
For example:
...
In this mode, the container file system and/or container file system squashed image produced by Detect Docker Inspector
only contains files added to the image by application layers. If the Black Duck SCA signature scanner is run on this file,
...
information, such as the operating system files that enable it to determine the Linux distribution.This could negatively affect its ability to accurately identify components.

## Considerations when running on Windows

### Docker file sharing settings

Detect Docker Inspector requires the ability to share directories with the image inspector containers.
...
The shared directories are created under the Detect output directory
(controlled by Detect *detect.output.path*).
If you change the location of the Detect output directory, be sure your Docker file sharing settings enable
...
Consequently, you cannot run Detect Docker Inspector on a Windows Docker image
...
to Detect Docker Inspector using the *docker.tar* property.

## Inspecting multiple images more efficiently by leaving services running

By default, Detect Docker Inspector starts, uses, and then stops and removes either one or two containerized
...
and it doesn't support concurrent execution of multiple Detect Docker Inspector runs.

The scanning of many images is completed significantly faster by starting the image inspector services
once, and running multiple instances of Detect Docker Inspector, so that each one sends requests to the already-running
...
If you invoke Detect Docker Inspector with an image reference; in other words, a repo:tag value versus a .tar file,
it uses the [docker-java library](https://github.com/docker-java/docker-java), to access the Docker registry to pull the image.

If you can pull an image using *docker pull* from the command line, then you will be able to configure Detect Docker Inspector to pull that image.
...
(and therefore Detect Docker Inspector) for your Docker registry,
...
When *use.platform.default.docker.host* is set to true (the default value) *and* Detect Docker Inspector is running
on Windows, Detect Docker Inspector overrides only the DOCKER_HOST value
...
1. Run Detect Docker Inspector on the target image to get the container file system.
   You can also do this using Detect using `--detect.docker.passthrough.*` properties.
   Include the following Detect Docker Inspector properties:
...
For example, if you are invoking Detect Docker Inspector from Detect, and want Detect
...
Detect Docker Inspector supports [OCI image](https://github.com/opencontainers/image-spec/blob/main/spec.md) archives (.tar files)
passed to it via the Detect `detect.docker.tar` property.

Detect Docker Inspector derives the image repo:tag value for each manifest in the index.json file from an
annotation with key `org.opencontainers.image.ref.name` if present.

If insufficient information exists in the image's manifest.json for the repo name and tag, Detect Docker Inspector will default to using the image name, (without the .tar extension), as the project name and "latest" as the project version in the BOM. These default values can be overridden by using the `detect.project.name` and `detect.project.version.name` properties respectively.

If the OCI archive contains multiple images, Detect Docker Inspector constructs the target repo:tag from the values of Detect properties `detect.docker.passthrough.docker.image.repo`
...
that has value matching the constructed target repo:tag. If a match is found, Detect Docker Inspector inspects the matching image. If no match is found, or `detect.docker.passthrough.docker.image.repo`
is not set, Detect Docker Inspector fails.
