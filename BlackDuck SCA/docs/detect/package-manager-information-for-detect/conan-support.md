---
title: "Conan support"
source_url: "https://docs.blackduck.com/r/detect/11.5.1/black-duck-detect/conan-support.html"
content_id: "~qn79eHFZuImKjE7JD5I6Q"
version: "11.5.1"
section: "Package Manager information for Detect"
scraped_at: "2026-08-08T23:44:51.412791+00:00"
---
...
Detect has two strategies for identifying components in Conan projects:
...
Detect will run a Conan Detector if either of the following is true:

- Detect finds or is provided via the *detect.conan.lockfile.path* property, a Conan lockfile. If no lockfile is provided, Detect looks for a file named *conan.lock*. In this case, the Conan Lockfile detector runs and discovers dependency details using the contents of the Conan lockfile. For Conan version 1.x, the Conan Lockfile detector is attempted first due to the additional information (package revisions) that may be provided by lockfiles. For Conan version 2.x, Conan CLI detector is attempted first as a Conan 2.x lockfile is a flat list of all components rather than a graph.
- Detect finds a file named *conanfile.txt* or *conanfile.py*. In this case, a Conan CLI detector runs and discovers dependency details by running the *conan info* or *conan graph info* command on the Conan project and parsing the output.

For Conan 1.x, in order for Detect to generate dependency details that will reliably match components
in the Black Duck KnowledgeBase, the Conan revisions feature must be enabled on the Conan project.
...
## Black Duck KnowledgeBase external ID generation
...
Detect constructs a Black Duck KnowledgeBase external ID for namespace "conan" using these fields as follows:
...
Detect has an alternative mode, package revision matching, that includes
the package ID and package revision in the Black Duck KnowledgeBase external IDs that it constructs (in addition to the fields described above).
...
In this scenario, Detect constructs a Black Duck KnowledgeBase external ID for namespace "conan" as follows:
...
Some Conan users use modified versions of Open Source packages with custom user/channel values. These modified components will not match components in the Black Duck KnowledgeBase.
The Black Duck KnowledgeBase requires a match on user and channel.
