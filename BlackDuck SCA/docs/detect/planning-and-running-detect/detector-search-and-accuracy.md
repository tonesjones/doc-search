---
title: "Detector search and accuracy"
source_url: "https://docs.blackduck.com/r/detect/11.5.1/black-duck-detect/detector-search-and-accuracy.html"
content_id: "ntW__L7OoCFB_yq7_RThMg"
version: "11.5.1"
section: "Planning and running Detect"
scraped_at: "2026-08-08T23:44:37.142172+00:00"
---
...
The graph is eventually included in Detect's output: the BDIO.
...
3. If an XCODE project has already processed, ignore XCODE project files that are nested inside.

Nesting rules can be disabled by setting property `detect.detector.search.continue` to true.

## Detector cascade

Detector cascade is a strategy designed to produce the most accurate results possible.
For a given project root directory, Detector cascade first tries the detector that would produce the most accurate results.
If the first detector is unable to run (if, for example, the package manager executable it needs is not on the PATH),
Detector cascade will try the next-best detector. This process continues until one of the applicable detector's extraction method succeeds
or Detect runs out of detectors that apply.

Detect will always try the more accurate detectors first, falling back to less accurate detectors only if the more accurate
detectors fail (or can't be run).

Cascade sequences are not configurable.

Detector cascade in combination with detector accuracy (described below) replace the previous (pre-Detect 8) distinction between "build mode" and "buildless mode",
...
If none succeed, Detect proceeds to the next (if there is one) for the Detector Type.

## Troubleshooting detector search

For more insight into the decisions Detect made during detector search, generate
...
3. The extract method discovers dependencies and returns a graph. In a few cases extraction is performed with the help of a separate Detect component called an inspector.
...
You can find the accuracy for each detector in the detector table.
...
Detect could run the Gradle Native Inspector detector
...
This property defaults to ALL, which means you want Detect to exit if any detector type applies, but
only low accuracy results could be generated.
When Detect exits due to accuracy requirements not being met, it returns the FAILURE_ACCURACY_NOT_MET exit code.
This default produces behavior roughly similar to the default mode (detect.detector.buildless=false) prior to Detect 8.

To get the best results available regardless of accuracy, set this property to NONE.
This value produces behavior roughly similar to buildless mode (detect.detector.buildless=true) prior to Detect 8,
...
If the user's accuracy requirements were not met, Detect
