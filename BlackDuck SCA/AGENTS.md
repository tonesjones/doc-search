...
9. **Protect shared SCA test capacity.** Treat 10 Active versions as the operational limit for the shared `Tony RAG` test project. Before creating a version, count the existing Active versions and stop if the addition would exceed 10. Never delete a version or convert it to LTS automatically; require a human to select obsolete test data or approve the one-way LTS conversion.
10. **Explain customer concepts relationally.** Do not stop at a circular or documentation-native label such as “base unit.” For an unfamiliar customer, use the cited documentation to explain what the entity represents and how it relates to its child or parent entities. Keep the explanation within what the source establishes.
11. **Separate configurable defaults from the correct customer choice.** When a field has a documented default but the documentation describes multiple lifecycle or delivery options, state the default and explain that the customer should select the option matching the project version's actual context. For project-version phase, use the organization's software-development lifecycle; for distribution, use how the application or product is released. Do not imply that the default is universally the right choice.
12. **Treat UI navigation as version-sensitive.** Prefer the most specific, current, version-compatible UI procedure. If two authoritative local sources give different paths and version ordering does not resolve the conflict, surface the conflict instead of silently combining or choosing one. Preserve the exact visible labels from the selected source. API-token UI navigation is shared by SCA and companion tools: for a request explicitly asking for the current or latest path, also search `docs/c-cpp-tool/black-duck-c-cpp-tool/api-token.md` and prefer its `latest` labels over an older version-pinned help-center path.
13. **Prefer self-describing C/CPP options in customer examples.** For a standard Black Duck C/CPP Tool command, use the documented long option names (`--build_cmd`, `--build_dir`, `--project_name`, `--project_version`, and `--bd_url`) unless the user specifically asks for short aliases. Keep optional signature-scan arguments out of the standard command unless the use case requires them.
