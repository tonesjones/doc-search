...
def write_hub_index() -> None:
    """Write/refresh root index.md as multi-product hub when companions exist."""
    rows = []
    ...
    # Prefer detailed SCA index if only phase-1 exists; otherwise hub + per-product
    ...
    a(
        ...
        "Per-product catalogs hold the full TOC; this hub tracks scrape progress."
    )
    a("")
    a("## Products")
    a("")
    ...
    # Keep SCA full TOC at index-blackduck.md and hub at index.md.
    # Actually current SCA is index.md. Strategy:
    # - SCA full catalog stays at its configured index_file (index.md historically)
    # - When companions ready, write hub to CORPUS.md and leave indexes separate
    #
...
def build_one(cfg: dict, args: argparse.Namespace) -> int:
    ...
    if rebuild_topics:
        ...
        # Backfill productKey if older SCA manifest
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Black Duck docs index/manifest")
    parser.add_argument(
        "--product",
        type=str,
    ...
    if args.hub or args.product == "all" or any(
        get_product(k).get("phase") == 2 for k in keys
    ):
        try:
            write_hub_index()
        except Exception as e:
