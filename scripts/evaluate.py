...
def main() -> int:
    ...
    eval_paths = [args.cases, args.profile, ROOT / "evaluation" / "schema" / "eval-case.schema.json", ROOT / "evaluation" / "core.py", Path(__file__)]
    ...
    effective_prompt_revision = next(iter(prompt_revisions)) if len(prompt_revisions) == 1 else "NOT_MEASURED"
    ...
    report = {
        ...
        "git_revision": provenance["checkout_revision"],
        "working_tree_dirty": provenance["checkout_dirty"],
        "corpus_revision": provenance["source_revision"],
        "evaluation_profile": provenance["evaluation_profile"],
        "entrypoint": provenance["entrypoint"],
        "instruction_revision": provenance["instruction_revision"],
